...
`Healthcare Software` `Applied AI` `Data Engineering` `Distributed Systems`
`Cloud Infrastructure` `Clinical Data` `Digital Health` `Deep Tech`
...
### Backend & Data
`Spring Boot` `REST APIs` `PySpark` `Databricks` `Pandas`

### ML / AI
`PyTorch` `TensorFlow` `LLMs` `LangChain` `RAG`

### Cloud & Infrastructure
`Microsoft Azure` `Docker` `Kubernetes` `CI/CD`
